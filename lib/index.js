...
packer.gulp = function (program, config) {
  ...
  gulp.task('watch', function () {
    ...
    if (program.watch) {
      ...
      gulp.watch(source.styles.watch,   ['css:styles:app']);
      gulp.watch(source.scripts.watch,  ['es6:scripts:app']);
      gulp.watch((source.views.watch.length)?(source.views.watch):(source.views.src),
                                        ['views:hbs:app']);
      gulp.watch((source.images.watch.length)?(source.images.watch):(source.images.src),
                                        ['images:styles:app']);
      gulp.watch((source.components.watch.length)?(source.components.watch):(source.components.src),
                                        ['components:hbs:app']);
    }
  ...
  gulp.task('default',
    ...
    function() {
      ...
      gutil.log(gutil.colors.cyan('* 编译完成 *'), ((program.watch)?('可以开始编辑代码啦！'):('')));
      gutil.log(gutil.colors.cyan('************'));
    })
    .on('task_stop', function(event) {
      var time = prettyTime(event.hrDuration);
      gutil.log('任务', chalk.cyan(event.task), '完成，耗时', chalk.gray(time));
  });
  ...
  /* 外部依赖 */
    ...
    if (true) {
      ...
      vendor.get(source.vendor, function () {
        console.log('gulp.start');
      })
      // /* 可以把 vendor 模块化 */
      // for (var i = 0; i < source.vendor.length; i++) {
      //    看一下版本是不是每次要更新 
      //   if (false) {
      //     continue;
      //   }
