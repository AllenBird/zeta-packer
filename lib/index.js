...
packer.gulp = function (program, config, callback) {
  ...
  var pathRelative = function(filePath) {
    ...
    var sep = path.sep
    var regexp = new RegExp('.*app\\' + sep + 'components|.*app\\' + sep + 'scripts|.*app\\' + sep + 'views')
    if (-1 != parse.dir.search(regexp)) {
      var comp = parse.dir.replace(regexp, '');
      var array = comp.split(sep)
      array.splice(0,1);
  ...
  gulp.task('hbs:app', gulpsync.sync(['views:hbs:app', 'components:hbs:app', 'precompile:hbs:app']));
  ...
  // 这个任务需要在public中的view ready之后执行
  gulp.task('precompile:hbs:app', function () {
    var files = source.precompile.src;
    var data = source.precompile.data;
    for (var i = 0; i < files.length; i++) {
      var uri = path.join(source.rootPath, files[i])
      if (fs.existsSync(uri)) {
        try{
         var fdata = _.template(fs.readFileSync(uri, 'utf8'))(data);
         fs.writeFileSync(uri, fdata, {encoding: 'utf8'});
        }
        catch (e){
